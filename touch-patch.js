...
(() => {
    ...
    window.__RAG_TOUCH_PATCH_BUILD__ = "v23-ui-toolkit";

    const BUILD = "v23-ui-toolkit";
    ...
    function ensureGeneratedBorderTexture(
        mods
    ) {
        const path =
            "rag_generated/auto_panel_border";
    ...
    function likelyBorderCandidates(
        headerPath
    ) {
        const normalized =
            normalizeTexture(
    ...
    async function detectBorderTexture(
        headerPath,
        mods
    ) {
        for (
            const candidate
            of likelyBorderCandidates(
                headerPath
            )
        ) {
            const existing =
        ...
        return ensureGeneratedBorderTexture(
            mods
        );
    }

    function findAutoPanelBorder(
        panel
    ) {
        const host =
            panelMainElement(
    ...
    async function createAutoPanelBorder(
        panel,
        borderPath,
        mods
    ) {
        ...
        border.canvasHolder.dataset.ragPanelBorderAuto =
            "true";

        border.canvasHolder.dataset.ragFixedDecorative =
            "true";

        border.canvasHolder.dataset.ragKeepAspect =
            "false";

        border.canvasHolder.style.pointerEvents =
            "none";

        border.canvasHolder.style.zIndex =
            "0";

        border.resizeHandle.style.display =
            "none";

        border.gridElement.style.pointerEvents =
            "none";
    ...
    async function createAutoHeaderAndBorder(
        ...
        const borderPath =
            await detectBorderTexture(
                headerPath,
                mods
            );

        await createAutoPanelBorder(
            panel,
            borderPath,
            mods
        );
        ...
        parent.dataset.ragAutoBorderTexture =
            normalizeTexture(
                borderPath
            );
        ...
        showBanner(
            headerState.json
                ? "Header + borda criados. Sidecar nativo detectado no header."
                : "Header + borda criados automaticamente.",
            "success"
        );
    }
    ...
    function patchCopyPasteAutoChrome(
        ...
        // Canvas metadata restoration already preserves ragPanelBorderAuto
        // and ragFixedDecorative through patchCopyPasteMetadata.
        ensureChromeResizeObserver(
            mods
        );
    }
