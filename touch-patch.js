...
`;
    ...
    function installGalleryButton() {
        ...
        message.textContent = "PNG, JPG, JPEG ou WEBP • abre o seletor de arquivos";

        const input = document.createElement("input");
        input.type = "file";
        // Sem accept: no Android isso abre o seletor geral de documentos/arquivos,
        // permitindo Arquivos, ZArchiver, ES File Explorer e outros providers.
        ...
        // Opcional: quem realmente quiser a galeria ainda pode usá-la.
        const galleryButton = document.createElement("button");
        galleryButton.type = "button";
        galleryButton.className =
            "rag-gallery-import-button rag-gallery-photo-button";
        ...
        galleryInput.addEventListener("change", () => {
            ...
            // Reuse the validated generic importer by transferring the selected
            // File through a DataTransfer when supported.
            try {
                ...
                // Fallback for browsers that do not allow assigning input.files:
                // process the gallery file directly using the same logic.
                (async () => {
                    const images = window.images;
                    ...
                    if (!(images instanceof Map)) {
                        setMessage(
                            wrapper,
                            "Mapa de imagens ainda nao carregou.",
                            "error"
                        );
                        return;
                    }
                    ...
                    galleryButton.textContent = "IMPORTANDO...";
                    setMessage(wrapper, file.name);
...
(() => {
                ...
                fontScale: Math.max(
                    ...
                    scale /
                        (
                            ...
                            (
                                Number(
                                    STATE.modules?.config?.magicNumbers
                                        ?.fontScalar
                                ) || 1.6
                            )
                        )
    ...
    function isEditorStyleButton(json) {
        if (!json || typeof json !== "object") return false;

        return (
            json.$button_size !== undefined ||
            json.$button_offset !== undefined ||
            json.$default_button_background_texture !== undefined ||
            json.$hover_button_background_texture !== undefined ||
            json.$pressed_button_background_texture !== undefined ||
            json.collection_index !== undefined
        );
    }
    ...
    function createControl(parentClass, rawKey, rawValue, parentVars, stack) {
        ...
        try {
            if (type === "image") {
                ...
                if (isEditorStyleButton(prepared.json)) {
                    ...
                    // Ordinary Bedrock button definitions are arbitrary
                    // containers with default/hover/pressed controls.
                    // Do NOT wrap them in the editor's special button model,
                    // otherwise it injects its own label/image and distorts
                    // external JSON UI.
    ...
    function directImportedChildren(instance) {
        const host = instance?.getMainHTMLElement?.();
        if (!host) return [];

        return [...host.children].filter((child) => {
            if (!(child instanceof HTMLElement)) return false;
            if (!child.dataset?.id) return false;
            if (child.dataset.skip === "true") return false;
            return true;
        });
    }

    function applyStackLayout(instance, json, vars) {
        const orientation = String(
            resolveVar(json?.orientation, vars) || "vertical"
        ).toLowerCase();

        const children = directImportedChildren(instance);
        ...
        for (const child of children) {
            ...
            if (orientation === "horizontal") {
                ...
                child.style.top =
                    `${Math.max(0, (hostRect.height - height) / 2)}px`;
                cursor += width;
            } else {
                child.style.left =
                    `${Math.max(0, (hostRect.width - width) / 2)}px`;
    ...
    function isNativeEditorJson(parsed) {
        ...
        return Boolean(
            ...
            namespace &&
            parsed[namespace] &&
            typeof parsed[namespace] === "object" &&
            parsed.config &&
            typeof parsed.config === "object" &&
            parsed.config.magicNumbers &&
            typeof parsed.config.magicNumbers === "object" &&
            Number.isFinite(
    ...
    function clearUniversalStateForNative() {
        STATE.parsed = null;
        STATE.rootKeys = [];
        STATE.currentRoot = "";
        STATE.rawByElementId.clear();
    ...
    async function importHybrid(raw, fileName) {
        ...
        if (isNativeEditorJson(parsed)) {
            clearUniversalStateForNative();

            // The original importer is still the most accurate renderer
            // for JSON exported by this editor because it understands its
            // own magicNumbers/font offsets/custom button format exactly.
            STATE.modules.FormUploader.uploadForm(raw);

            showImportBanner(
                `JSON aberto em modo COMPATIVEL: ${fileName}`,
                "success"
            );
        ...
        await universalImport(raw, fileName);

        showImportBanner(
            `JSON aberto em modo UNIVERSAL: ${STATE.currentRoot}`,
            "success"
        );
...
.draggable-panel[data-rag-imported-type="stack_panel"] {
...
`;
    ...
    async function installUniversalImporter() {
        ...
        const runUniversalUpload = function () {
            const input =
                document.getElementById("form_importer");
            ...
            showImportBanner(
                `Lendo ${file.name} com importador automatico...`
            );
            ...
            reader.onload = async () => {
                try {
                    if (window.__RAG_BUILTIN_TEXTURES_READY__) {
                        await window.__RAG_BUILTIN_TEXTURES_READY__;
                    }
        ...
        // Replace the old public method too, for callers that invoke
        // Builder.uploadForm() directly.
        Builder.uploadForm = runUniversalUpload;
        Builder.__ragUniversalUploadInstalled = true;

        // CRITICAL V7 FIX:
        // The original HTML has onchange="Builder.uploadForm()".
        // Some browsers/WebViews kept the old inline handler compiled even
        // after Builder.uploadForm was replaced. Remove it completely and
        // intercept the change event in capture phase so FormUploader.isValid()
        // never gets a chance to reject ordinary Bedrock JSON UI.
        if (
            formInput &&
            formInput.dataset.ragUniversalCapture !== "true"
        ) {
            formInput.dataset.ragUniversalCapture = "true";
            ...
            const label = formInput.closest("label");

            if (
                label &&
                !label.querySelector(".rag-universal-active-badge")
            ) {
                const badge = document.createElement("span");
                badge.className = "rag-universal-active-badge";
                badge.textContent = " AUTO";
                badge.title =
                    "Importador automatico: compativel + universal";
                label.appendChild(badge);
            }
        }

        console.log(
            "[Touch Patch v7] Universal JSON UI importer instalado e handler antigo removido."
        );
    }
...
/* RAG TOUCH PATCH BUILD: V10-HYBRID-IMPORT */
window.__RAG_TOUCH_PATCH_BUILD__ = "v10-hybrid-import";

/* ============================================================
 * JSON-UI Maker - Smart Texture Resolver V8
 *
 * - Preloads built-in preset textures (red, eternal, pink,
 *   turquoise, other) into the editor.
 * - Resolves missing paths by basename automatically:
 *     assets/red_default_button
 *       -> presets/textures/red_ore-ui_style/red_default_button
 * - Only uses a generated fallback when no matching texture exists.
 * - Suppresses the old "Image ... not found" cascade by making
 *   FileUploader.isFileUploaded() resolve/alias the path first.
 * - Replacing an image preserves exact left/top/width/height/layer
 *   AND preserves the current box aspect ratio for future locked resize.
 * ============================================================ */
(() => {
    "use strict";

    if (window.__JSON_UI_MAKER_SMART_TEXTURE_V8__) return;
    window.__JSON_UI_MAKER_SMART_TEXTURE_V8__ = true;
    ...
    window.__RAG_TEXTURE_FALLBACK_PATHS__ = FALLBACK_PATHS;
    ...
    async function imageUrlToImageData(url) {
        ...
        try {
            ...
            await new Promise((resolve, reject) => {
                img.onload = resolve;
                img.onerror = () =>
                    reject(new Error(`Falha ao abrir ${url}`));
                img.src = objectUrl;
            });
    ...
    function generateFallbackImage(path) {
        const existing =
            mods?.index?.images?.get("assets/placeholder")?.png;
        ...
        ctx.fillStyle = "rgba(0,0,0,.82)";
        ctx.fillRect(0, 91, 128, 37);

        ctx.fillStyle = "#fff";
        ctx.font = "bold 9px sans-serif";
        ctx.textAlign = "center";
        ctx.fillText(
            basename(path).slice(0, 20) || "MISSING",
            64,
            112
        );
    ...
    function candidateForPath(requested) {
        const images = mods.index.images;
        const targetBase = basename(requested);
        ...
        for (const [key, state] of images.entries()) {
            if (!state?.png) continue;
            if (FALLBACK_PATHS.has(key)) continue;
            if (basename(key) !== targetBase) continue;
        ...
        // Prefer exact preset names, then shortest path.
        matches.sort((a, b) => {
            const aPreset = a.key.includes("/presets/") ||
                a.key.startsWith("presets/");
            const bPreset = b.key.includes("/presets/") ||
                b.key.startsWith("presets/");
    ...
    function resolveOrFallback(requestedPath) {
        const requested = normalize(requestedPath);
        const images = mods.index.images;

        const direct = images.get(requested);
        if (direct?.png && !direct.__ragFallback) {
            FALLBACK_PATHS.delete(requested);
            return {
                path: requested,
                state: direct,
                mode: "exact",
        ...
        const candidate = candidateForPath(requested);

        if (candidate) {
            // Alias the original JSON path to the real texture state.
            images.set(requested, candidate.state);
            FALLBACK_PATHS.delete(requested);

            return {
                path: requested,
                state: candidate.state,
                sourcePath: candidate.key,
                mode: "alias",
            };
        }

        const fallback = {
            png: generateFallbackImage(requested),
            __ragFallback: true,
            __ragRequestedPath: requested,
        };

        images.set(requested, fallback);
        FALLBACK_PATHS.add(requested);
    ...
    async function preloadPreset(setName) {
        const mapping = await fetchJson(
            `presets/textures/${setName}/mapping.json`
        );

        const rows = Array.isArray(mapping?.data)
            ? mapping.data
            : [];
        ...
        await Promise.all(
            rows.map(async (row) => {
                ...
                const canonical =
                    `presets/textures/${setName}/${name}`;

                if (mods.index.images.get(canonical)?.png) return;

                try {
                    const png = await imageUrlToImageData(
                        `presets/textures/${setName}/${name}.png`
                    );
                    ...
                    if (row.nineslice) {
                        try {
                            json = await fetchJson(
                                `presets/textures/${setName}/${name}.json`
                            );
                        } catch (_) {}
                    }
                    ...
                    const state = {
                        png,
                        json,
                        __ragBuiltinPreset: setName,
                    };

                    mods.index.images.set(canonical, state);

                    // Useful aliases for external JSON UIs.
                    const aliases = [
                        `assets/${name}`,
                        `ui/${name}`,
                    ];
                    ...
                    for (const alias of aliases) {
                        const existing = mods.index.images.get(alias);

                        if (!existing?.png || existing.__ragFallback) {
                            mods.index.images.set(alias, state);
                            FALLBACK_PATHS.delete(alias);
                        }
                    }
                } catch (error) {
                    console.warn(
                        `[Smart Texture V8] Nao carregou ${setName}/${name}:`,
                        error
                    );
                }
            })
    ...
    async function preloadAllPresets() {
        if (window.__RAG_BUILTIN_TEXTURES_READY__) {
            return window.__RAG_BUILTIN_TEXTURES_READY__;
        }

        window.__RAG_BUILTIN_TEXTURES_READY__ =
            Promise.allSettled(
                PRESET_SETS.map(preloadPreset)
            ).then(() => {
                console.log(
                    "[Smart Texture V8] Presets internos carregados."
                );
                return true;
            });
    ...
    function preserveGeometry(instance, callback) {
        const el = instance.canvasHolder;
        ...
        const geometry = {
            ...
            keepAspect: el.dataset.ragKeepAspect,
            currentAspect:
                (parseFloat(el.style.width) || rect.width || 1) /
                Math.max(
                    1,
                    parseFloat(el.style.height) ||
                    rect.height ||
        ...
        el.style.left = geometry.left;
        el.style.top = geometry.top;
        el.style.zIndex = geometry.layer;
        ...
        // IMPORTANT:
        // Preserve the ELEMENT'S current ratio, not the new PNG's ratio.
        // This prevents a texture swap from changing later proportional resize.
        instance.aspectRatio = geometry.currentAspect;

        return geometry;
    }

    function patchFileUploader() {
        const FileUploader = mods.FileUploader;

        if (FileUploader.__ragSmartTextureV8) return;
        FileUploader.__ragSmartTextureV8 = true;

        const original = FileUploader.isFileUploaded.bind(
            FileUploader
        );

        FileUploader.isFileUploaded = function (fileName) {
            ...
            resolveOrFallback(normalized);

            // The old importer now sees the requested path as available,
            // so it does not rewrite it to assets/placeholder and does not
            // create a warning for every missing texture.
            return true;
        };
    }

    function patchCanvasReplacement() {
        const Canvas = mods.DraggableCanvas;

        if (Canvas.prototype.__ragSmartTextureReplaceV8) return;
        Canvas.prototype.__ragSmartTextureReplaceV8 = true;

        const original = Canvas.prototype.changeImage;

        Canvas.prototype.changeImage = function (imagePath) {
            const requested = normalize(imagePath);
            const resolved = resolveOrFallback(requested);

            // Ensure the original method sees the state under requested path.
            mods.index.images.set(
                requested,
                resolved.state
            );

            preserveGeometry(this, () => {
                original.call(this, requested);
            });

            this.canvasHolder.dataset.imagePath =
                requested;

            if (resolved.mode === "fallback") {
                this.canvasHolder.dataset.ragFallbackActive =
                    "true";
                this.canvasHolder.dataset.ragMissingTexture =
                    requested;
            } else {
                this.canvasHolder.dataset.ragFallbackActive =
                    "false";
            ...
            if (resolved.sourcePath) {
                this.canvasHolder.dataset.ragResolvedTexture =
                    resolved.sourcePath;
            } else {
                delete this.canvasHolder.dataset.ragResolvedTexture;
            }
        };
    }

    function patchButtonTextureReplacement() {
        const Button = mods.DraggableButton;

        if (Button.prototype.__ragSmartButtonTextureV8) return;
        Button.prototype.__ragSmartButtonTextureV8 = true;

        const originalDefault =
            Button.prototype.setDefaultImage;

        const originalHover =
            Button.prototype.setHoverImage;

        const originalPressed =
            Button.prototype.setPressedImage;

        Button.prototype.setDefaultImage = function (path) {
            const requested = normalize(path);
            resolveOrFallback(requested);

            const oldW =
                parseFloat(this.button.style.width) ||
                this.button.getBoundingClientRect().width;
            const oldH =
                parseFloat(this.button.style.height) ||
                this.button.getBoundingClientRect().height;

            originalDefault.call(this, requested);

            if (oldW && oldH) {
                this.drawImage(
                    oldW,
                    oldH,
                    this.imageDataDefault
                );
            }
        };

        Button.prototype.setHoverImage = function (path) {
            const requested = normalize(path);
            resolveOrFallback(requested);
            originalHover.call(this, requested);
        };

        Button.prototype.setPressedImage = function (path) {
            const requested = normalize(path);
            resolveOrFallback(requested);
            originalPressed.call(this, requested);
        };
    }

    function markFallbackElements() {
        const root = document.getElementById("main_window");
        if (!root) return;

        const scan = () => {
            root
                .querySelectorAll(
                    ".draggable-canvas[data-image-path]"
                )
                .forEach((el) => {
                    ...
                    if (
                        FALLBACK_PATHS.has(path) ||
                        state?.__ragFallback
                    ) {
                        el.dataset.ragFallbackActive =
                            "true";
                        el.dataset.ragMissingTexture =
                            path;
                    } else {
                        el.dataset.ragFallbackActive =
                            "false";
    ...
    async function initSmartTextures() {
        try {
            const [
                index,
                filesMod,
            ...
            mods = {
                ...
                DraggableCanvas:
                    canvasMod.DraggableCanvas,
                DraggableButton:
                    buttonMod.DraggableButton,
            };

            // Patch synchronously first. Even if preset preloading is still
            // running, missing paths will get a real fallback instead of fail.
            patchFileUploader();
            patchCanvasReplacement();
            patchButtonTextureReplacement();
            markFallbackElements();

            await preloadAllPresets();

            // Upgrade any fallback that now has a matching built-in texture.
            for (const path of [...FALLBACK_PATHS]) {
                const candidate = candidateForPath(path);
                if (!candidate) continue;

                mods.index.images.set(path, candidate.state);
                FALLBACK_PATHS.delete(path);
            }

            console.log(
                "[Touch Patch v8] Smart texture resolver pronto."
            );
        } catch (error) {
            console.error(
                "[Touch Patch v8] Smart texture resolver falhou:",
                error
            );
        }
    }
