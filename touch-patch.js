...
(() => {
    ...
    function persistNineSliceForPath(path, nineSlice) {
        if (!validNineSlice(nineSlice)) return;
            ...
            localStorage.setItem(
                ...
                JSON.stringify({
                    ...
                    metadata: {
                        ...
                        relativePath: `textures/${normalizeTexture(path)}.json`,
                        matchedFromNativeAsset: true,
                    },
                })
            );
        } catch (_) {}
    ...
    async function applyImportedImageRole(instance, role, mods) {
        if (!instance) return;

        const element = instance.canvasHolder;
        const parent = instance.container;
        const path = normalizeTexture(element.dataset.imagePath);
        const state = migrateAutomaticNineSlice(
            path,
            mods.index.images.get(path)
        );

        if (state?.png && !validNineSlice(state.json)) {
            const nativeSidecar = await discoverNativeNineSlice(
                mods,
                path,
                state.png,
                state
            ...
            if (nativeSidecar) {
                state.json = nativeSidecar;
                persistNineSliceForPath(path, nativeSidecar);
            }
        }

        if (role === "image") {
            const box = currentBox(instance);
            instance.nineSlice = validNineSlice(state?.json)
                ? cloneNineSlice(state.json)
                : undefined;
        ...
        instance.nineSlice = roleNineSlice(state, role, mods);
    ...
    async function installImageParentLock() {
        ...
        Builder.addCanvas = function (...args) {
            ...
            void applyImportedImageRole(added, role, mods);
    ...
    function autoRoleNineSlice(imageData, role) {
        if (role === "button_texture") {
            return autoButtonNineSlice(imageData);
        }
        ...
        const maxX = Math.max(1, Math.floor((width - 1) / 2));
        const maxY = Math.max(1, Math.floor((height - 1) / 2));
        const side = Math.min(maxX, 8, Math.max(1, Math.round(width / 6)));
        const top = Math.min(maxY, 8, Math.max(1, Math.round(height / 6)));
        const bottom = Math.min(
            Math.max(1, height - top - 1),
            15,
            Math.max(1, Math.round(height * 5 / 12))
        ...
        return {
            nineslice_size: [side, top, side, bottom],
            base_size: [imageData.width, imageData.height],
            __ragRoleGenerated: role,
        };
    }

    function cloneNineSlice(value) {
        if (!validNineSlice(value)) return undefined;

        return {
            ...value,
            nineslice_size: Array.isArray(value.nineslice_size)
                ? [...value.nineslice_size]
                : value.nineslice_size,
            base_size: Array.isArray(value.base_size)
                ? [...value.base_size]
                : value.base_size,
        };
    }
    ...
    function findLoadedNativeNineSlice(mods, imageData, ownState = null) {
        for (const state of mods.index.images.values()) {
            if (
                state === ownState ||
                !validNineSlice(state?.json) ||
                state.json.__ragAutoGenerated ||
                state.json.__ragRoleGenerated ||
                !sameImageData(imageData, state.png)
            ) {
                continue;
            }

            return cloneNineSlice(state.json);
        }

        return undefined;
    }

    async function discoverNativeNineSlice(mods, path, imageData, ownState = null) {
        const loaded = findLoadedNativeNineSlice(mods, imageData, ownState);
        if (loaded) return loaded;

        const catalog = PRESET_CATALOG.get(baseName(path));
        if (!catalog?.[1]) return undefined;

        const [setName, , actualName] = catalog;
        const preset = await fetchImageState(
            setName,
        ...
        if (
            preset?.png &&
            validNineSlice(preset.json) &&
            sameImageData(imageData, preset.png)
        ) {
            return cloneNineSlice(preset.json);
        }

        return undefined;
    }

    function roleNineSlice(state, role, mods) {
        if (!state?.png) return undefined;
        ...
        const matched = findLoadedNativeNineSlice(mods, state.png, state);
        if (matched) return matched;

        state.__ragRoleNineSlices ||= {};
        state.__ragRoleNineSlices[role] ||= autoRoleNineSlice(
            state.png,
            role
        );

        return cloneNineSlice(state.__ragRoleNineSlices[role]);
    }

    async function resolveRoleNineSlice(mods, path, state, role) {
        if (!state?.png) return undefined;

        migrateAutomaticNineSlice(path, state);

        if (!validNineSlice(state.json)) {
            const nativeSidecar = await discoverNativeNineSlice(
                mods,
                path,
                state.png,
        ...
        return roleNineSlice(state, role, mods);
    }
    ...
    async function importChosenFile(wrapper, file, sidecarFile = null) {
        ...
        const requestedRole = buttonTextureImportContext()
            ? "button_texture"
            : selectedImageRole();
        const nativeSidecar =
            sidecar ||
            await discoverNativeNineSlice(
                mods,
                file.name,
                imageData
            );
        const generatedRoleSidecar =
            !nativeSidecar &&
            ["button_texture", "background", "border", "header"]
                .includes(requestedRole)
                ? autoRoleNineSlice(imageData, requestedRole)
                : null;
        const roleNineSlices =
            generatedRoleSidecar && requestedRole !== "button_texture"
                ? { [requestedRole]: generatedRoleSidecar }
                : undefined;

        mods.index.images.set(key, {
            png: imageData,
            json: nativeSidecar || undefined,
            __ragButtonNineSlice:
                requestedRole === "button_texture"
                    ? generatedRoleSidecar || undefined
                    : undefined,
            __ragRoleNineSlices: roleNineSlices,
        });

        // A pixel-identical preset sidecar is native metadata, so persist it
        // exactly like a manually selected JSON sidecar.
        persistImage(key, file, dataUrl, nativeSidecar);
        ...
        status.textContent =
            ...
            (sidecar
                ? " • 9-SLICE DO JSON"
                : nativeSidecar
                ? " • 9-SLICE IDENTICO AO ASSET"
                : generatedRoleSidecar
                ? ` • 9-SLICE ${requestedRole.toUpperCase()}`
                : "");
    ...
    async function installImageResizeAndReplacement() {
        ...
        if (!Canvas.prototype.__ragV11ImagePatch) {
            ...
            Canvas.prototype.changeImage = function (path) {
                ...
                const role =
                    this.canvasHolder.dataset.ragBorderSystemRole ||
                    (this.canvasHolder.dataset.ragHeaderBackground === "true"
                        ? "header"
                        : "");

                if (role) {
                    this.nineSlice = roleNineSlice(state, role, mods);
                }
    ...
    async function createEditableHeader(
        ...
        const headerNineSlice = await resolveRoleNineSlice(
            mods,
            texturePath,
            state,
            "header"
    ...
    async function createBorderSystemCanvas(
        ...
        const canvasNineSlice =
            await resolveRoleNineSlice(
                mods,
                texturePath,
                state,
                role
        ...
        roleEditorSelect.addEventListener("change", async () => {
            ...
            try {
                if (role === "header") {
                    ...
                    await applyImportedImageRole(instance, role, mods);
                    showBanner(
                        role === "background"
