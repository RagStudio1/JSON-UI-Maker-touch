...
(() => {
    ...
    window.__RAG_TOUCH_PATCH_BUILD__ = "v36-button-nine-slice-import";

    const BUILD = "v36-button-nine-slice-import";
                ...
                localStorage.setItem(
                    ...
                    JSON.stringify({
                        ...
                        metadata: {
                            ...
                            importedFromFiles: true,
                            autoGenerated: Boolean(nineSlice.__ragAutoGenerated),
                        },
                    })
                );
            }
    ...
    function persistGeneratedNineSlice(key, nineSlice) {
        try {
            localStorage.setItem(
                `asset_${key}_json`,
                JSON.stringify({
    ...
    async function importChosenFile(wrapper, file, sidecarFile = null) {
        ...
        mods.index.images.set(key, {
            png: imageData,
            json: nineSlice || undefined,
        });

        persistImage(key, file, dataUrl, nineSlice);
    ...
    function patchCompoundButtons(mods) {
        ...
        Button.prototype.drawImage = function (
            ...
            if (!imageDataState?.json && imageDataState?.png) {
                ...
                // it behave like the project's native button assets.
                if (normalizeTexture(texturePath).startsWith("gallery/")) {
                    imageDataState.json = autoButtonNineSlice(
                        imageDataState.png
                    );
                    mods.index.images.set(
                        normalizeTexture(texturePath),
                        imageDataState
                    );
                    persistGeneratedNineSlice(
                        normalizeTexture(texturePath),
                        imageDataState.json
                    );
                }
